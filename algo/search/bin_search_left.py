...
- search the leftmost occurance
""" 

DEBUG = 0
def_cmprator = lib.utils.get_def_cmprator()

def bin_search_left(elemlist, target, \
    ...
    if start_idx <= end_idx:
        ...
        if target_idx == -1:
            target_idx = bin_search_left(elemlist, target,\
                                          start_idx, end_idx,\
                                          cmprator)
...
def main(argv=None): 
    ...
    target_idx = bin_search_left(input['list'], input['target'])
